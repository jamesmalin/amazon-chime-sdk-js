...
export default class DefaultDeviceController implements DeviceControllerBasedMediaStreamBroker {
  ...
  private videoInputDeviceCount: number = 0;
  ...
  private async chooseInputDevice(
    ...
    this.videoInputDeviceCount += 1;
    const callCount = this.videoInputDeviceCount;

    if (device === null && kind === 'video') {
      this.lastNoVideoInputDeviceCount = this.videoInputDeviceCount;
