...
export default class DefaultDeviceController implements DeviceControllerBasedMediaStreamBroker {
  ...
  private async chooseAudioInputDeviceInternal(device: Device): Promise<DevicePermission> {
    ...
    try {
      ...
      if (device) {
        deviceSelection = await this.getDeviceSelection(kind, device, proposedConstraints);
      } else {
    ...
    if (stream) {
      ...
      deviceSelection.stream.getTracks()[0].addEventListener('ended', () => {
        if (this.activeDevices[kind].stream === deviceSelection.stream) {
          ...
          if (kind === 'audio') {
            this.chooseAudioInputDeviceInternal(null);
          } else {
            this.chooseVideoInputDeviceInternal(null, false);
          }
        }
      });
    }
